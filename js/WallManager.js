...
class WallManager {
  ...
  constructor(gameWidth, gameHeight) {
    ...
    // 벽 Y 위치 (지면에 맞춤)
    // Wall 클래스의 height = 300과 동일하게 설정
    // groundBottomOffset를 캐릭터의 groundOffset(250)과 동일하게 설정
    const groundBottomOffset = 250;
    const wallHeight = 300;
    this.wallY = gameHeight - groundBottomOffset - (wallHeight / 2);
  ...
  updateAndDisplayHitEffects() {
    ...
    this.activeHitEffects = this.activeHitEffects.filter(effect => {
      ...
      if (frame) {
        ...
        const targetSize = 375; // 벽 높이 기준 / 사이즈 150 -> 300 으로 변경
