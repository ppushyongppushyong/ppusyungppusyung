...
class WallManager {
  ...
  constructor(gameWidth, gameHeight) {
    ...
    // 3. Y 위치 계산: (바닥 위치) - (벽 높이의 절반)
    // 이유: imageMode(CENTER)라서 중심점이 y가 되므로, 절반만큼 위로 올려야 바닥에 닿음
    this.wallY = groundY - (wallHeight / 2);
    ...
    this.wallY = gameHeight - 200 - 225; // 캐릭터 발 위치에서 벽 높이/2 뺀 위치
